...
def patch_image_sn_discriminator(images, ndf=64):
    batch_size = images.shape[0].value
    layers = []
    paddings = [[0, 0], [1, 1], [1, 1], [0, 0]]
    ...
    # Tranform to 1 channel
    with tf.variable_scope("sn_conv4_0"):
        layers.append(conv2d(layers[-1], 1, kernel_size=3, strides=1))

    with tf.variable_scope("sn_fc4"):
        logits = tf.reduce_mean(tf.reshape(layers[-1], (batch_size, -1)), axis=-1)
        layers.append(logits)
    return layers

def video_sn_discriminator(clips, ndf=64):
    # (T, N, H, W, C) -> (N, T, H, W, C)
    # ndf is the initial #discriminator channels. 2x spatial reduction -> 2x ndf increase
